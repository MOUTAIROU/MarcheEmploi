...
import "./style.css";
import { TYPE_DISPLAY_MAP, currency, CATEGORIE_LABELS } from '@/utils/types';
...
const AnnonceList: React.FC<Props> = ({ annonces, total, currentpage }) => {
    ...
    function getCategorieSlug(key: string): string {
        const label = CATEGORIE_LABELS[key];
        if (!label) return "";
        ...
        return label
            ...
            .replace(/&/g, " ")               // remplace &
            .replace(/[^a-z0-9\s-]/g, "")     // supprime spéciaux
            .trim()
            .replace(/\s+/g, "-");            // espaces → -
    }
    ...
    function getAnnonceHref(a: Annonce) {
        ...
        const ville = a.lieu ? a.lieu.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
            : "toutes-villes";

        if (a.type === "offre_emploi") {
            return `${process.env.LOCAL_HOST}/annonces/emploi/${ville}/${getCategorieSlug(categorie || '')}/${slug}`;
        }

        // tout le reste = appel d’offre
        return `${process.env.LOCAL_HOST}/annonces/appel-offre/${ville}/${getCategorieSlug(categorie || '')}/${slug}`;
