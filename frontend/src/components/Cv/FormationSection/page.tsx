...
export interface FormationSectionRef {
  fillFormationFromParent: (newFormations: Formation[]) => void;
...
const FormationSection = forwardRef<FormationSectionRef>((_, ref) => {
  ...
  const [formationActuelle, setFormationActuelle] = useState<Formation>({
    id: Date.now(),
    titre: "",
    etablissement: "",
    ville: "",
  ...
  useImperativeHandle(ref, () => ({
    fillFormationFromParent(newFormations: Formation[]) {
    ...
    getFormations() {           // ✅ nouvelle méthode exposée
      return formations;
    },
  }));
