...
export default function CreerOffre() {
    ...
    const [typeContratValue, setTypeContratValue] = useState("");
    const [niveauEtudeValue, setNiveauEtudeValue] = useState("");
    ...
    async function getOffres() {
        try {
            const response = await api.get(`entreprise_get/emploi_offres_by_post_id/${post_id}`);
            ...
            // Champs texte
            if (titreRef.current) titreRef.current.value = offre.objet || "";
            ...
            setSelectValue("categorie", offre.categorie);
            setSelectValue("lieu", offre.lieu);
            setSelectValue("visibilite", offre.visibilite);
            setSelectValue("niveau_experience", offre.experience);
            setSelectValue("type_contrat", offre.typeContrat);
            setSelectValue("niveau_etude", offre.niveauEtudes);

            setTypeContratValue(offre.typeContrat?.toUpperCase());
            setNiveauEtudeValue(offre.niveauEtudes);

            // Date
            const setInputDate = (name: string, value: string) => {
                const input = document.querySelector(`input[name="${name}"]`) as HTMLInputElement | null;
                if (input && value) input.value = value.slice(0, 10); // YYYY-MM-DD
            };

            setInputDate("expiration", offre.expiration);

            // Quill
            if (descriptionRef.current?.root) descriptionRef.current.root.innerHTML = offre.description || "";
            if (conditionsRef.current?.root) conditionsRef.current.root.innerHTML = offre.conditions || "";
    ...
    async function submitOffre(payload: FormData) {
        ...
        const response = await api.post("/entreprise/update_offre_emploi", payload, {
                        ...
                        <div>
                            <form className="form" onSubmit={handleSubmit}>
                                <label>
                                    ...
                                    <textarea name="titre" ref={titreRef} />
                                </label>
                                ...
                                <div className="row">
                                    ...
                                    <label>
                                        ...
                                        <select name="type_contrat"
                                            value={typeContratValue}
                                            onChange={(e) => setTypeContratValue(e.target.value)}>
                                ...
                                <div className="row">
                                    <label>
                                        ...
                                        <select name="niveau_etude"
                                            value={niveauEtudeValue}
                                            onChange={(e) => setNiveauEtudeValue(e.target.value)}>
