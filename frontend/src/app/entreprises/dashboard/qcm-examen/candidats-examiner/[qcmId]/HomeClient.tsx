...
export default function OffresPage() {
    ...
    const renderScore = (status: string, score?: number | null) => {
        if (status === "ASSIGNED") {
            return <span className="muted">Indisponible</span>;
        }
        ...
        if (score >= 70) className += "score-good";
        else if (score >= 50) className += "score-medium";
        else className += "score-bad";
    ...
    const renderTime = (status: string, seconds?: number) => {
        if (status === "ASSIGNED") {
            return <span className="muted">Indisponible</span>;
        }
        ...
        return <span className="time">{formatTime(seconds)}</span>;
    };


    const renderFraude = (status: string, count?: number) => {
        if (status === "ASSIGNED") {
            return <span className="muted">Indisponible</span>;
        }

        if (!count || count === 0) {
            return <span className="fraude-safe">✔️ Aucun incident</span>;
        }

        if (count <= 2) {
            return <span className="fraude-warning">⚠️ Faible ({count})</span>;
        }

        if (count <= 5) {
            return <span className="fraude-warning">⚠️ Moyen ({count})</span>;
        }

        return <span className="fraude-danger">🚨 Élevé ({count})</span>;
    };
                        ...
                        <table className="table">
                                ...
                                <tr>
                                    <th>
                                        <input
                                            ...
                                            checked={
                                                selectedOffres.length === offres.length &&
                                                offres.length > 0
                                            }
                                        />
                                    </th>
                            ...
                            <tbody>
                                {offres.map((offre) => {
                                    ...
                                    const fraudeCount = offre.antiFraudeLogs ?? 0;
                                    ...
                                    return (
                                        <tr key={offre.user_id}>
                                            ...
                                            <td className="candidat-name">
                                                <strong>{offre.nom || "Non défini"}</strong>
                                            </td>

                                            {/* EMAIL */}
                                            <td className="email">
                                                {offre.email ? (
                                                    <span title={offre.email}>
                                                        {offre.email.length > 25
                                                            ? offre.email.slice(0, 25) + "..."
                                                            : offre.email}
                                                    </span>
                                                ) : (
                                                    "-"
                                            ...
                                            <td>
                                                <span
                                                    className={`score-badge ${score >= 70
                                                        ? "score-good"
                                                        : score >= 50
                                                            ? "score-medium"
                                                            : "score-bad"
                                                ...
                                                >
                                                    {renderScore(offre.status, score)}
                                            ...
                                            <td>
                                                <span className="time">
                                                    ...
                                                    {renderTime(offre.status, offre.tmp)}
                                                </span>
                                            </td>
                                            ...
                                            {/* ANTI-FRAUDE */}
                                            <td>
                                                {renderFraude(offre.status, fraudeCount)}
                                            </td>
                                                    ...
                                                    <div className="dropdownRow" ref={rowMenuRef}>
                                                        {Rowactions.map((action) => (
                                                            <div
                                                                ...
                                                                className="dropdownItem"
                                                                onClick={() =>
                                                                    handleRowAction(action, offre.user_id)
                                                                }
                                                            >
                                                                {action}
                                                            </div>
