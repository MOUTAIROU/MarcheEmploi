...
export default function OffresPage() {
    ...
    useEffect(() => {
        ...
        setStartTime(Date.now()); // timestamp en millisecondes
    }, [examStarted]);
    ...
    }, [questionCourante, examTermine, examStarted]);
    ...
    }, [tempsRestant, examTermine, examStarted]);


    const logAntiFraude = (type: AntiFraudeEvent["type"]) => {
        ...
        setAntiFraudeLogs((prev) => [
            ...prev,
            {
                type,
                timestamp: Date.now(),
                question: params?.modePassage !== "libre" ? questions[currentIndexRef.current].intitule : "Indisponible"
            },
        ]);
    ...
    async function getOffres() {
        try {
            const response = await api.get(
                `candidats/get_qcm_by_post_id/${qcmId}`
            );
    ...
    const handleReponseChange = (questionId: number, value: any) => {
        ...
        setReponses((prev: Record<number, any>) => ({
            ...prev,
            [questionId]: value, // clé = questionId, valeur = réponse
        }));
    };
            ...
            <main >
                <div className="container-dashbord">
                    ...
                    <div className="mainContent">
                        ...
                        <QcmIntroModal
                            isOpen={!examStarted && questions.length > 0}
                            titre={reverse_treatment_msg(titre)}
                            description={reverse_treatment_msg(description)}
                            duree={Number(duree)}
                            mode={params?.modePassage}
