...
export default function Home() {
    ...
    async function fetchPreferences() {
        try {
            const res = await api.get("/users/get_cand_profile_settings");

            console.log("Réponse settings :", res.data);

            if (!res.data?.data) return;

            const data = res.data.data;

            setVisibleProfil(toBool(data.visibleProfil));
            setContactDirect(toBool(data.contactDirect));
            setPublicProfil(toBool(data.publicProfil));

            setDisplayBio(toBool(data.displayBio));
            setDisplaySkills(toBool(data.displaySkills));
            setDisplayExperience(toBool(data.displayExperience));

            setShowCV(toBool(data.showCV));
            setOpenToWork(toBool(data.openToWork));
